(function () {
  ...
  angular.module('com.bonitasoft.features.admin.processes.actorsMapping', [
    'ui.bootstrap',
    'ui.router',
    'angular-growl',
    ...
    .controller('actorsMappingCtrl', function ($scope, $modal, process, actorMemberAPI, actorAPI, growl) {
      ...
      resourceInit.pagination = {currentPage: 1, numberPerPage: 10};

      $scope.actors = {resource: resourceInit};
      $scope.actorsMember = [];
      $scope.actorsMembers = [];
      var actorUserFilterDeploy = {filter: 'member_type=USER', deploy: ['user_id']};
      var actorGroupFilterDeploy = {filter: 'member_type=GROUP', deploy: ['group_id']};
      var actorRoleFilterDeploy = {filter: 'member_type=ROLE', deploy: ['role_id']};
      var actorMembershipFilterDeploy = {filter: 'member_type=MEMBERSHIP', deploy: ['role_id', 'group_id']};
      ...
      self.deleteMember = function deleteMember(memberId) {
        actorMemberAPI.delete({id: memberId}).$promise.then(
          function success() {
            growl.success('Actor member deleted', {ttl: 3000, disableCountDown: true, disableIcons: true});
            self.getActors();
          },
          function error() {
